using System.Collections.Generic;

namespace ElimpParse.DatabaseProvider
{
    public class TaskPackStorage
    {
        public static readonly List<int> TasksAGroup = new List<int>
        {
            5133,
            7401,
            7944,
            4716,
            133,
            138,
            923,
            108,
            1623,
            927,
            1118,
            542,
            4538
        };

        public static readonly List<int> TasksBGroup = new List<int>
        {
            4717,
            7943,
            519,
            902,
            8242,
            248,
            20,
            903,
            1312,
            7548
        };

        public static readonly List<int> TasksCGroup = new List<int>
        {
            7293,
            7336,
            2391,
            67,
            905,
            7460,
            518,
            421,
            407,
            904,
            2218,
            11,
            125
        };

        public static readonly List<int> TasksDGroup = new List<int>
        {
            2,
            4805,
            280,
            340,
            7813,
            920,
            2862,
            910,
            382,
            473,
            500,
            7410
        };

        public static readonly List<int> TasksEGroup = new List<int>
        {
            922,
            2098,
            4760,
            1952,
            1965,
            7402
        };

        public static readonly List<int> TasksFGroup = new List<int>
        {
            922,
            2098,
            4760,
            1952,
            1965,
            7402
        };
    }
}